...
describe("POST /login", () => {
  ...
  it("Should return 400 with email address or password does not exist within out database", async () => {
    ...
    expect(response.body.message).toMatchObject([{ path: ["email"], message: "Email address or password is not correct" }])
...
describe("POST /register", () => {
  ...
  it("Should return 200 with user info and tokens", async () => {
    ...
    const response = await request(makeApp()).post("/api/register").send({
      ...userPayload,
      password: "password",
...
describe("POST /refresh", () => {
  ...
  it("Should return 200 with tokens", async () => {
    ...
    const response = await request(makeApp()).post("/api/refresh").set({
  ...
  it("Should return 401 with unauthorized token expires", async () => {
    const user = await database.user.create({ data: userPayload })
    const token = jwt.sign({ id: user.id }, config.SECRET_REFRESH, { expiresIn: 0 });
...
describe("POST /forgot-password", () => {
  ...
  it("Should return 200 with If the email is present in our database, a corresponding email will be sent to it. when email exists", async () => {
    ...
    const response = await request(makeApp()).post("/api/forgot-password").send({ email: user.email })
  ...
  it("Should return 200 with If the email is present in our database, a corresponding email will be sent to it. when does not exists", async () => {
    const response = await request(makeApp()).post("/api/forgot-password").send({ email: "test@test.com" })
...
describe("POST /send-confirmation-email", () => {
  ...
  it("Should return 204", async () => {
    ...
    const response = await request(makeApp()).post("/api/send-confirmation-email").set({
      "authorization": `Bearer ${token}`
    });
...
describe("POST /reset-password/:token", () => {
  ...
  it("Should return 204", async () => {
    ...
    const response = await request(makeApp()).post(`/api/reset-password/${token}`).send({
      password: "password",
      password_confirmation: "password"
    })
  ...
  it("Should return 400 with jwt expired", async () => {
    const user = await database.user.create({ data: userPayload })
    const token = jwt.sign({ id: user.id }, config.SECRET_FORGOT_PASSWORD, { expiresIn: 0 });
...
describe("POST /confirm-email/:token", () => {
  ...
  it("Should return 204", async () => {
    ...
    const response = await request(makeApp()).post(`/api/confirm-email/${token}`).set("authorization", `Bearer ${accessToken}`);
  ...
  it("Should return 400 with jwt expired", async () => {
    ...
    const user = await database.user.create({ data: userPayload });
    const token = jwt.sign({ id: user.id }, config.SECRET_CONFIRM_EMAIL, { expiresIn: 0 });
    const accessToken = jwt.sign({ id: user.id }, config.SECRET_ACCESS);
...
describe("GET /me", () => {
  ...
  it("Should return 200 with user", async () => {
    ...
    const response = await request(makeApp()).get("/api/me").set({
      "authorization": `Bearer ${token}`
    });
...
describe("PATCH /me", () => {
  ...
  it("Should return 204", async () => {
    ...
    const response = await request(makeApp()).patch("/api/me").set("authorization", `Bearer ${accessToken}`).send({
...
describe("PATCH /change-password", () => {
  ...
  it("Should return 204", async () => {
    ...
    const response = await request(makeApp()).patch("/api/change-password").set("authorization", `Bearer ${accessToken}`).send({
      password: "password",
